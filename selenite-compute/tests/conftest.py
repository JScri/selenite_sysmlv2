...
def _module_results(module_name: str, script: str):
    """Run a port module once per session; returns (ported, results_or_reason).
    A module that ports several scripts sharing one workspace exposes
    ``run_script(name)`` returning the workspace as captured after that script."""
    ...
    try:
        if hasattr(mod, "run_script"):
            return True, mod.run_script(script)
        return True, mod.run()
...
def results_for(row: goldens.GoldenRow):
    return _module_results(row.module, row.script)
