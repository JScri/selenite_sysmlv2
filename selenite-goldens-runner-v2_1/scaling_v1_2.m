...
%    - PKT KREEP transport: chemical SKIPs P7-P9, surface haulers P10+
%      Eliminates the ice bottleneck (chemical SKIP propellant demand
%      was 953 Mt/yr at P12 — exceeds all lunar ice)
%    - Ice mining depth: 10m (was 1-2m). Strip mining with MOLE-S.
%    - PROBE fleet scaling for PGM: larger vehicles + in-situ beneficiation
%    - Earth transit propellant: crew Starship return fuel from ISRU
%    - Cargo Starships: one-way expendable (no return fuel needed)
%    - EM catapult hubs for ore transport >50 km at P11+
%    - Ni/Fe demand tracked (in-space structural use, not Earth return)
%
...
fprintf('════════════════════════════════════════════════════════════════════════\n');
fprintf('  SELENITE SCALE v1.2 — 100-Year Dual-Base Scaling Analysis\n');
...
% EM catapult hub (for ore transport >50 km at P11+)
catapult.range_km = 100; catapult.v_launch = 500; % m/s
catapult.canister_kg = 5000; catapult.pwr_MW = 5; % per launch
...
for i = 1:ph.n
    ...
    % ═══ EM CATAPULT HUBS (P11+ for long-range ore at PKT) ═══
    if i >= 5 % P11+
        % Assume 30% of ore comes from >50 km (needs catapult)
        ore_catapult = ph.raw_kreep(i) * 0.30;
        ph.n_catapults(i) = ceil(ore_catapult/1000 / (catapult.canister_kg/1000 * catapult.launches_hr * 8760 * 0.80));
    else
        ph.n_catapults(i) = 0;
    end
    ...
    ph.pwr_catapult(i) = ph.n_catapults(i)*catapult.pwr_MW*1000*0.30; % 30% avg duty
    ph.pwr_pkt(i) = (ph.pwr_proc_pkt(i)+ph.pwr_hauler(i)+ph.pwr_bene_pkt(i)+...
                     ph.pwr_catapult(i)+20)*1.20; % +20 kW SENTINEL/comms
...
fprintf('══════════════════════════════════════════════════════════════════════\n');
fprintf('  %-12s %10s %8s %6s %10s %8s\n',...
    'Phase','Haulers','Catapults','Bene','PKT kW','FSP');
fprintf('  %s\n',repmat('-',1,60));
for i=1:ph.n
    fprintf('  %-12s %10s %8d %6d %10s %8d\n',...
        ph.lab{i}, sprintf('%.0f',ph.n_haulers(i)), ph.n_catapults(i),...
        ph.bene_total(i), sprintf('%.0f',ph.pwr_pkt(i)), ph.fsp_pkt(i));
end
...
for i=1:ph.n
    ...
    fprintf('  SKIPs:    %6d SPA + %8d PKT | Haulers: %s | Catapults: %d\n',...
        ph.skip_spa(i), ph.skip_pkt(i), sprintf('%.0f',ph.n_haulers(i)), ph.n_catapults(i));
...
fprintf('\n═══════════════════════════════════════════════════════════════════════\n');
fprintf('  v1.1 → v1.2 COMPARISON: Ice Demand Reduction\n');
fprintf('═══════════════════════════════════════════════════════════════════════\n');
% v1.1 P12 MOLE-I was 194,838,070. v1.2 should be dramatically less.
fprintf('  v1.1 P12 MOLE-I: 194,838,070 (chemical SKIPs at PKT)\n');
fprintf('  v1.2 P12 MOLE-I: %s (surface haulers at PKT)\n', sprintf('%.0f',ph.n_molei(ph.n)));
fprintf('  Reduction: %.1f%%\n', (1-ph.n_molei(ph.n)/194838070)*100);
fprintf('  v1.1 P12 ice sites: 120,608\n');
fprintf('  v1.2 P12 ice sites: %d\n', ph.ice_sites(ph.n));
fprintf('  v1.1 P12 SPA power: 1,159,645,289 kW (1.16 TW)\n');
fprintf('  v1.2 P12 SPA power: %s kW\n', sprintf('%.0f',ph.pwr_spa(ph.n)));

%% ═══ FIGURES ═══
figure('Position',[50 50 1500 950],'Color','k','Name','SELENITE SCALE v1.2');
...
subplot(2,3,2);
bar(1:ph.n, ph.n_haulers, 'FaceColor',[.2 .8 .5]);
set(gca,'XTick',1:ph.n,'XTickLabel',ph.lab,'Color',[.04 .06 .1],'XColor','w','YColor','w','FontSize',8);
xtickangle(30); ylabel('Count','Color','w'); title('PKT Surface Haulers (P10+)','Color','w');
grid on; set(gca,'GridColor',[.2 .25 .35]);
...
sgtitle('SELENITE v1.2 — Surface Haulers + Deep Ice + PROBE Scaling','Color','w','FontSize',14,'FontWeight','bold');

fprintf('\n═══════════════════════════════════════════════════════════════════════\n');
fprintf('  ANALYSIS COMPLETE — v1.2\n');
fprintf('  KEY: Surface haulers eliminate PKT propellant bottleneck\n');
fprintf('  Ice demand reduced by >99%% vs v1.1\n');
fprintf('  PROBE fleet scales for PGM demand with in-situ beneficiation\n');
fprintf('═══════════════════════════════════════════════════════════════════════\n');
