...
**SEL-DECISION-001 Rev C | April 2026 | DRAFT**
...
**Changes from Rev B:** Committed REO target revised from 1.25 Mt/yr to 2.5 Mt/yr (full replacement of terrestrial REE mining). Optimised continuous-flow processing circuit (1.2 t/yr, 28,000 kg, 400 kW) replaces batch design — 3× circuit count reduction. PROBE Mk III SEP architecture (ion propulsion, never lands, EM anchoring) with Mk I/II phasedown. Base specialisation codified: SPA = all asteroid processing, PKT = KREEP only. Diversified asteroid captures (one M-type, one C-type, one S-type — not multiple M-types). Laser Ablation Truss for captured asteroid mining. Fifth mass driver (DRO→SPA). Economic evaluation integrated (BCR 0.50/2.06, NPV −$0.49T, SS +$121.8B/yr). Construction fleet scaling formulae. P13/P14/P14+ phases added with Earth independence trajectory. Circuit Earth fraction trajectory to 0.8% by Y135.
...
**Every km² of PKT surface is valuable KREEP ore.** No non-KREEP infrastructure is built on it. PKT has: processing circuits, haulers, conveyors, SINTER, MSR for KREEP power. No PROBE operations, no asteroid processing, no PGM. PKT is fully autonomous — zero permanent crew. Temporary 3–4 person commissioning crew at P8 only (Wave 3, 6–12 months for foundry/MSR commissioning, departs before fuel loading).
...
4. **SEL-POWER-PKT-001 Rev A** — new document required: PKT thorium MSR power architecture
